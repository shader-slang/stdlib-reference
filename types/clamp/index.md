...
### Conformance to IForwardDifferentiable\<clamp\<T, M, N\>\>
`<T, int N>` additionally conforms to `IForwardDifferentiable<clamp<T, M, N>>` when the following conditions are met:
