...
`<T>` additionally conforms to `IForwardDifferentiable<log2<T, N>>` when the following conditions are met:
  ...
  * [T](index.html#typeparam-T) : [\_\_BuiltinFloatingPointType](../../interfaces/0_builtinfloatingpointtype-029hm/index.html)
### Conformance to IForwardDifferentiable\<log2\<T, M, N\>\>
`<T>` additionally conforms to `IForwardDifferentiable<log2<T, M, N>>` when the following conditions are met:
