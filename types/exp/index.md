...
`<T>` additionally conforms to `IForwardDifferentiable<exp<T, N>>` when the following conditions are met:
  ...
  * [T](index.html#typeparam-T) : [\_\_BuiltinFloatingPointType](../../interfaces/0_builtinfloatingpointtype-029hm/index.html)
### Conformance to IForwardDifferentiable\<exp\<T, M, N\>\>
`<T>` additionally conforms to `IForwardDifferentiable<exp<T, M, N>>` when the following conditions are met:

  * [T](index.html#typeparam-T) : [\_\_BuiltinFloatingPointType](../../interfaces/0_builtinfloatingpointtype-029hm/index.html)
### Conformance to IBackwardDifferentiable\<exp\<T\>\>
`<T>` additionally conforms to `IBackwardDifferentiable<exp<T>>` when the following conditions are met:
...
`<T>` additionally conforms to `IBackwardDifferentiable<exp<T, N>>` when the following conditions are met:
  ...
  * [T](index.html#typeparam-T) : [\_\_BuiltinFloatingPointType](../../interfaces/0_builtinfloatingpointtype-029hm/index.html)
### Conformance to IBackwardDifferentiable\<exp\<T, N, M\>\>
